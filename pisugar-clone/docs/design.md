...
| 电感要求 | 1µH, Idc>4.5A, DCR<0.01Ω(官方手册) | 选 Coilcraft XGL4040-102(DCR 5.6mΩ/Isat 9.3A),远超要求,见 BOM |
...
选型按**最优解**(性能/精度优先;货源由淘宝 + 立创保障,不为缺货妥协)。被动件/结构件的 LCSC·JLC 料号已逐条核实(幻觉料号已剔除);标 *unclear* 的为下单时再确认。
...
| 位号 | 器件 | 规格 | 料号 | 备注 |
|---|---|---|---|---|
| L1 | 功率电感 | 1µH, DCR **5.6mΩ**(max), Isat 9.3A, Irms 14.8A, 4×4mm, AEC-Q200 | Coilcraft XGL4040-102MEC **C3911436** | 金属粉末模压,业界最低 DCR 级,**远超**官方 Idc>4.5A/DCR<10mΩ;电源电感最优解(进口,立创可订)。国产替代:顺络/Codaca 金属粉末 1µH 低 DCR 款 |
| R_sense | 采样电阻 | 0.01Ω 1% **合金 2512**,低 TCR | Viking RS-12FL4-0R01 **C2920591**(AEC-Q200 车规合金) | CSIN/CSIN_S 走**开尔文**布线;极致精度可上真 4 端 Kelvin(Stackpole CSNL2512FT10L0 / Vishay WSK2512,料号下单确认) |
...
**与官方/草案差异**:① 删 X1 晶振(SD3078 内置);② 采样电阻用**合金 2512 + 开尔文布线**(精度最优);③ 删 40pin 排针→ **pogo 顶针**;④ 删 NTC 分压(按官方 R7 NC);⑤ 新增 R_CC(USB-C 受电下拉)、SW1 滑动开关。
